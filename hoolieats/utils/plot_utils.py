...
def generate_profile_md(df: DataFrame):
    # operating_hours_sub_pattern = r"\[|\]|'"
    print(df.head())
    return f"""
        ```
        Name: {df['name'].iloc[0]}
        Operating Hours:
            - Mon-Fri   : {df['opening_hours'].iloc[0]}
            - Sat       : {df['opening_hours'].iloc[0]}
            - Sun       : {df['opening_hours'].iloc[0]}
        Parking Lot: {', '.join(df['parking_lot'].iloc[0]).capitalize()}
        Payment Method(s): {', '.join(df['payment_method'].iloc[0]).capitalize().replace('_', ' ')}
        Cuisine(s): {', '.join(df['cuisine'].iloc[0]).capitalize().replace('_', ' ')}
        ...
        URL: {df['url'].iloc[0]}
        Ambience: {df['ambience'].iloc[0].capitalize()}
...
def restaurant_user_map(restaurant_loc_df, initial_view, marker_size: Tuple):
    user_loc_df = query_executor.execute("SELECT user_latitude, user_longitude FROM users")
    map = pdk.Deck(
        ...
        layers=[
            pdk.Layer(  # user location layer
                "ScatterplotLayer",
                user_loc_df,
                get_position=["user_longitude", "user_latitude"],
                auto_highlight=True,
            ...
            pdk.Layer(  # restaurant location layer
                ...
                get_position="[place_longitude, place_latitude]",

...
def app():
    ...
    total_no_customers = pd.read_sql_query(
        'SELECT COUNT(DISTINCT("userID")) FROM cview', con=engine
    )["count"].iloc[0]
    st.write(
        "### Total number of unique customers: **" + str(total_no_customers) + "**"
    )

    ## First row
    col1, col2 = st.columns(2)
    # age distribution
    age_dist_df = pd.read_sql_query(customer_age, con=engine)
    age_dist_fig = age_dist_cache(age_dist_df)
    col1.plotly_chart(age_dist_fig, use_container_width=True)
    # bmi
    bmi_df = pd.read_sql_query(customer_bmi_status, con=engine)
